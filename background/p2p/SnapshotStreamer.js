...
class SnapshotStreamer {
  ...
  send(obj) {
    ...
    if (this.lastSendTimeMs) {
      console.log(
        `DEBUG p2p sending screenshot. Size: ${(bin.length / 1000).toFixed(
          2
        )}kb, last send (secs): ${(
          (Date.now() - this.lastSendTimeMs) /
          1000
        ).toFixed(2)}`
      );
    } else {
  ...
  async sendScreenshot() {
    ...
    chrome.tabs.get(this.streamingTab, (tab) => {
      ...
      if (tab.active) {
        this.getScreenshot(tab.windowId)
          .then(([img, tabId]) => {
        ...
        screenshotPersister.get(tab.id, (cachedShot) => {
          if (cachedShot) {
            ...
            this.send(
              createScreenshotMessage(cachedShot, tab.id, tab.windowId)
            );
          } else {
            this.send({ type: "screenshot_unavailable" });
            console.log(
              "Could not upload screenshot, tab is inactive and there is no cached screenshot"
            );
          }
        });
      }
  ...
  startStreaming = (tabId, windowId) => {
    ...
    this.streamInterval = setInterval(() => {
      if (
        ...
        console.log(
          `DEBUG p2p buffer NOT low, skipping screenshot. Buffered: ${(
            this.datachannel.bufferedAmount / 1000
          ).toFixed(2)}kb, buffer_low_mark: ${(
            this.datachannel.bufferedAmountLowThreshold / 1000
          ).toFixed(2)}`
        );
      }
    }, 1000);
        ...
        (img) => {
          if (chrome.runtime.lastError) {
            if (
              ...
              reject(
                new ScreenshotRateLimitError(
                  "Exceeded MAX_CAPTURE_VISIBLE_TAB_CALLS_PER_SECOND"
                )
              );
              return;
            } else {
