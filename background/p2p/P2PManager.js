...
class P2PManager {
   ...
   initSignaler(signalToken, channelName, remotePeerId) {
      ...
      if (!this.signalers[channelName] || ['closed', 'failed'].includes(this.signalers[channelName].client.connection.state)) {
   ...
   connect(signaler, remotePeerId, connectMsg) {
      ...
      peerConnection.pc.addEventListener("connectionstatechange", pcConnectionStateUpdate);
   ...
   closeChannelsWithPeer(remotePeerId) {
      if (this.channels[remotePeerId]) {
         ...
         if (remainingChannels.length === 0 && this.signalers[channelName] && this.signalers[channelName].client.connection.state === 'connected') {
