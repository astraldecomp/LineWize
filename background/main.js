...
const CAPTURE_TAB_EXCEPTIONS_URL = [CHAT_WINDOW_URL];
...
addListeners = () => {
  chrome.extension.onMessage.addListener((request, sender, sendResponse) => {
    if (request.config === "chat_status") {
      ...
      if (showChatBubble) {
        ...
        if (
          currentActiveClass &&
          currentActiveClass.hasOwnProperty("chat_blocked")
        ) {
      ...
      sendResponse({
        userDetails: config.getCurrentUserInfo(),
  ...
  chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    logging__message(
      "chrome.tabs.onUpdated called with tab status",
      changeInfo["status"]
    );
    ...
    if (changeInfo["status"] === "complete") {
      ...
      chrome.tabs.executeScript({
        file: "/background/events/message_scripts/createChatBubble.js",
      });
  ...
  chrome.tabs.onRemoved.addListener((tabId, removeInfo) => {
    if (config.isFilteringEnabled()) {
      tabs.tab_removed(
        removeInfo["windowId"] + "_" + tabId,
        config.getActiveConfigurations()
      );
    }
  });
        ...
        for (let configuration of config.getActiveConfigurations()) {
          if (is_active(configuration)) {
            response.classes.push({
              ...
              teacher_information: safeMapGet(
                configuration,
                "teacher_information",
                null
              ),
              ...
              is_monitoring_class: safeMapGet(
                configuration,
                "is_monitoring_class",
                false
              ),
  ...
  config.retrieve_configuration(
    ...
    () => {
      ...
      if (!hadActiveClassBeforeUpdate && hasActiveClass) {
        logging__message(
          "Class was not active before, now it is, taking screenshot",
          config
        );
        tabs.capture_tab_and_send();
      }
      logging__message(
        "config.retrieve_configuration finished, about to call loginLook callback",
        config
      );
      login.login();
...
fzbox_poll = setInterval(function () {
  ...
  xhr.onreadystatechange = () => {
    ...
    if (config.lastFZProbeCode !== xhr.status) {
      logging__warning(
        "[http://fzbox.tools] Network safety changed:",
        xhr.status === 200
          ? "Current network is safe"
          : "Current network is unsafe"
      );
    ...
    try {
      ...
      if (config.lastFZProbeProvider !== networkProvider) {
        logging__warning(
          "[http://fzbox.tools] Network provider changed:",
          networkProvider
        );
