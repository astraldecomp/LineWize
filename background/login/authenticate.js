...
class Authenticate {
  ...
  async partial() {
    ...
    if (!applianceId || (!username && !email)) {
      throw new AuthNotPossibleError(
        "applianceId and username or email is required for partial auth"
      );
    } else if (this.partialFailedCache.contains(applianceId, username, email)) {
      throw new AuthNotPossibleError(
        "partial auth credentials used are in the known bad credentials cache"
      );
    }

    try {
      await easyFetch(`${config.apiDispatcherUrl}/authenticate/agent/partial`, {
        method: "POST",
        body: JSON.stringify({ email, username, appliance_id: applianceId }),
      });
    } catch (error) {
      if (
        error.name === "EasyFetchStatusError" &&
  ...
  async google() {
    return new Promise((resolve, reject) => {
      chrome.identity.getAuthToken({ interactive: true }, async (token) => {
        if (!token) {
          reject(
            new UserRejectedSignInError("User did not complete Google sign-in")
          );
          return;
        }
        ...
        try {
          const response = await easyFetch(
            `${config.apiDispatcherUrl}/authenticate/agent/chrome/google`,
            {
              method: "POST",
              body: JSON.stringify({
                token,
                appliance_id: config.getDeviceId(),
              }),
            }
          );
  ...
  async onNetwork() {
    ...
    const username = config.currentUserInfo.user;
    await easyFetch(
      `${config.apiDispatcherUrl}/authenticate/agent/on-network?username=${username}`,
