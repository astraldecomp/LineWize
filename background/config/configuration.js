class Config {
  constructor() {
    ...
    this.regions = ["syd-1", "syd-2", "uk-1", "beta-1", "sit"];
    ...
    this.screenshot_max_width = 300;
    this.compressed_screenshots = true;
    this.screenshot_jpeg_quality = 0.1;
    this.classroom_chat_enabled = false;
    this.timezone = "";
    this.reauth_allowed_timer_seconds = undefined;
    this.asyncVerdictHandlingEnabled = false;
  }
  ...
  /**
   * @returns {number} pixel of what should be screenshot width to upload
   */
  getScreenshotMaxWidth() {
    return this.screenshot_max_width;
  }
  ...
  getYoutubeSafeSearchMode() {
    ...
    if (this.inside_device_network) {
      if (
        this.safeSearch_on_network_config &&
        this.safeSearch_on_network_config.youtube &&
      ...
      if (
        this.safeSearch_off_network_config &&
        this.safeSearch_off_network_config.youtube &&
  ...
  isBingSafeSearchEnabled() {
    if (this.inside_device_network) {
      if (
        this.safeSearch_on_network_config &&
        this.safeSearch_on_network_config.bing
      ) {
      ...
      if (
        this.safeSearch_off_network_config &&
        this.safeSearch_off_network_config.bing
      ) {
  ...
  isGoogleSearchEnabled() {
    if (this.inside_device_network) {
      if (
        this.safeSearch_on_network_config &&
        this.safeSearch_on_network_config.google
      ) {
      ...
      if (
        this.safeSearch_off_network_config &&
        this.safeSearch_off_network_config.google
      ) {
  ...
  updateDeviceLocation(next) {
        ...
        if (xhr.status !== 404) {
          ...
          try {
            ...
            if (userStatus["loggedin"] && userStatus["user"]) {
              ...
              next(
                true,
                self.inside_device_network__user,
                userStatus["device_id"],
                userStatus["region"]
              );
              return;
            }
    ...
    xhr.onerror = function () {
      logging__error(
        "Device location probe failed due to network error, setting state to off-network"
      );
      self.inside_device_network__authenticated = false;
      self.inside_device_network = false;
      next(false);
              ...
              chrome.identity.getProfileUserInfo((userinfo) => {
                if (userinfo && userinfo.email) {
                  ...
                  self.request_userid(
                    userinfo.email,
                    undefined,
                    (user, region, device_id) => {
                      if (oldUser !== user) {
                        // 5 minutes
                        logging__message(
                          "Identity not supplied by network device and user has changed, using Google account"
                        );
                        userinfo.user = user;
                        ...
                        storedSettings.device_id &&
                        storedSettings.userInfo &&
                        storedSettings.active_region
                      ) {
                        logging__message(
                          "Identity not supplied by network device but stored settings were found, using storage",
                          storedSettings
                        );
                ...
                logging__error(
                  "Cannot determine user, no information provided from chrome or network"
                );
              });
            });
          }
      ...
      if (xhr.readyState === 4) {
        ...
        if (xhr.status === 200) {
          try {
            ...
            if (
              dict_response["parent_device"] &&
              dict_response["parent_device"] !== dict_response["device_id"]
            ) {
              logging__message(
                "Config was for child device, retrieving parent config"
              );
            ...
            self.allow_connections_inside_network =
              dict_response["allow_connections_inside_network"];
            self.closedTabUrl = dict_response["closed_tab_url"];
            self.google_allow_insecure_chrome =
              dict_response["google_allow_insecure_chrome"];
            self.google_classroom_extension_login =
              dict_response["google_classroom_extension_login"];
            self.enable_extension_chromebooks_only =
              dict_response["enable_extension_chromebooks_only"];
            self.mobile_agent_config = self.extractFilteringAndClassroomConfig(
              dict_response["mobile_agent_config"]
            );
            self.userInformation = dict_response["user_information"];
            self.timezone = dict_response["timezone"];
            self.chromebook_screenshot_upload_interval_seconds =
              dict_response["chromebook_screenshot_upload_interval_seconds"];
            self.screenshot_max_width =
              dict_response["screenshot_max_width"] || 300;
            self.compressed_screenshots =
              dict_response["compressed_screenshots"] || true;
            self.screenshot_jpeg_quality =
              dict_response["screenshot_jpeg_quality"] || 0.1;
            self.is_teacher = dict_response["is_teacher"];
            self.reauth_allowed_timer_seconds =
              dict_response["reauth_allowed_timer_seconds"];
            self.asyncVerdictHandlingEnabled =
              dict_response["async_verdict_handling"] || false;

            // use feature flag to enable chat instead
            const featureFlags = dict_response["feature_flags"];
            self.classroom_chat_enabled =
              featureFlags && featureFlags["classwize-teacher-student-chat"];
  ...
  extractFilteringAndClassroomConfig(serverConfig) {
    ...
    if (serverConfig) {
      ...
      self.filter_on_network_enabled = onNetworkConfig["filtering"]
        ? true
        : false;
      if (self.filter_on_network_enabled) {
        self.safeSearch_on_network_config =
          onNetworkConfig["filtering"]["safeSearch"];
      }
      self.classroom_on_network_enabled =
        onNetworkConfig["classroom"]["enabled"];
      self.filter_off_network_enabled = offNetworkConfig["filtering"]
        ? true
        : false;
      if (self.filter_off_network_enabled) {
        self.safeSearch_off_network_config =
          offNetworkConfig["filtering"]["safeSearch"];
      }
      self.classroom_off_network_enabled =
        offNetworkConfig["classroom"]["enabled"];
    }
  }
  ...
  request_userid(user, deviceid, callback, errorCallback) {
    ...
    if (self.active_region) {
      ...
      let url =
        Config.get_gateway_url(self.active_region) +
        "/get/configuration/userid?identity=" +
        user;
      self.send_userid_request(
        self.active_region,
        url,
        callback,
        errorCallback
      ...
      for (let region of self.regions) {
        logging__message(
          "Unsure which region this user is in, searching",
          region,
          user
        );
        let url =
          Config.get_gateway_url(region) +
          "/get/configuration/userid?identity=" +
          user;
  ...
  send_userid_request(region, config_gateway_url, callback, errorCallback) {
    ...
    if (
      self.pendingUserIdCalls[region] &&
      current_timestamp - 5000 < self.pendingUserIdCalls[region]
    ) {
      logging__message("Get userId request is already in flight, skipping...");
      return;
    }
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4) {
        self.pendingUserIdCalls[region] = 0;
        if (xhr.status === 200) {
          let user;
          let device_id;
          try {
          ...
          errorCallback();
          logging__warning(
            "failed to retrieve User Id from the cfg-gateway, error was " +
              xhr.responseText,
            xhr
          );
        }
    ...
    self.pendingUserIdCalls[region] = current_timestamp;
    xhr.send();
  }
  ...
  retrieve_configuration(errorCallback, ...callbacks) {
    ...
    self.retrieve_userConfig((newUserIdentity = false) => {
      if (self.getCurrentUserInfo().user) {
        ...
        let configUrl =
          Config.get_gateway_url(self.active_region) +
          "/get/configuration/chrome-extension" +
          "?user=" +
