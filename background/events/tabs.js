class Tabs {
        ...
        function (img) {
          if (chrome.runtime.lastError) {
            if (
              ...
              logging__error(
                "captureVisibleTab failed, exceeded MAX_CAPTURE_VISIBLE_TAB_CALLS_PER_SECOND"
              );
            } else {
          ...
          sourceImage.onload = function () {
            const width = config.getScreenshotMaxWidth();
            ...
            // if the image width exceeds our maximum width, then we downscale
            const finalWidth =
              sourceImage.width > width ? width : sourceImage.width;
            const finalHeight =
              finalWidth * (sourceImage.height / sourceImage.width);

            oc.width = finalWidth;
            oc.height = finalHeight;

            // this downscales the image if the original exceeds the max. width
            // introduces artifacts / reduces the clarity of the image
            octx.drawImage(sourceImage, 0, 0, finalWidth, finalHeight);

            // send the image as a compressed jpeg if compression is enabled, otherwise it defaults to PNG
            if (config.compressed_screenshots) {
              callback(
                oc.toDataURL("image/jpeg", config.screenshot_jpeg_quality)
              );
            } else {
  ...
  capture_tab_and_send(pageComplete = false) {
    ...
    chrome.tabs.query({}, (queriedAllTabs) => {
      ...
      const allTabs = queriedAllTabs.filter(
        (tab) => !CAPTURE_TAB_EXCEPTIONS_URL.includes(tab["url"])
      );
      ...
      chrome.windows.getLastFocused((lastFocused) => {
        ...
        if (segment["url"].toLowerCase().startsWith("chrome")) {
          logging__debug(
            `${segment["url"]} is the active tab, skipping screenshot`
          );
        ...
        if (self.lastScreenshotUrl === segment["url"]) {
          ...
          if (!self.pageCompleteSentForURL && pageComplete) {
            ...
            logging__debug(
              `Uploading the 1st pageComplete screenshot for ${segment["url"]}`
            );
          } else if (self.lastScreenshotTime + 10 > nowInSeconds()) {
            logging__debug(
              `Rapid s'shot upload for ${segment["url"]}, skipping screenshot`
            );
            return;
          }
        } else {
              ...
              xhr.onreadystatechange = function () {
                if (xhr.readyState === 4) {
                  ...
                  if (xhr.status === 200) {
                    ...
                    logging__error(
                      "Failed to upload screen shot of tab",
                      segment
                    );
                  }
  ...
  should_lock(configuration) {
    return (
      config.getLockUrl() &&
      configuration.locked_users.includes(configuration.identity)
    );
  }
  ...
  prevent_navigation_handler(details) {
    if (
      details.url.indexOf("linewize.net") >= 0 ||
      details.url.startsWith("chrome-extension://" + chrome.runtime.id)
    ) {
      return {};
    }
  ...
  prevent_navigation() {
    if (
      !chrome.webRequest.onBeforeRequest.hasListener(
        this.prevent_navigation_handler
      )
    ) {
  ...
  updateActiveConfigurations(activeConfigurations) {
    ...
    if (config.isClassroomEnabled()) {
      ...
      if (runningConfigs.length === 0) {
        ...
        logging__message(
          "no running configs, stopping lock and focus if they are active"
        );
        this.stop_lock();
        this.stop_focus();
      } else {
      ...
      logging__message(
        "classroom functionality is disabled, stopping lock and focus if they are active"
      );
      this.stop_lock();
      this.stop_focus();
    }
   ...
   */
  maybe_deactivate_focus_and_or_lock(
    isConfigApplyingFocus,
    isConfigApplyingLock
  ) {
  ...
  scheduleFocusAndLockCheckAtNextClassEnd() {
    ...
    this.runningConfigs.forEach((c) => {
      ...
      const todayPeriods = c.periods.filter((p) => {
        ...
        if (timeout) {
          ...
          const periodEnd = date.set({
            hour: p.endTime / 100,
            minute: p.endTime % 100,
            second: 0,
            millisecond: 0,
          });
      ...
      if (timeout) {
        ...
        for (const p of todayPeriods) {
          const periodStart = date.set({
            hour: p.startTime / 100,
            minute: p.startTime % 100,
            second: 0,
            millisecond: 0,
          });
          const periodEnd = date.set({
          ...
          if (timeoutMs >= periodStart && timeoutMs <= periodEnd) {
            // if timeout is between the period start and end
        ...
        if (!resolvedTimeout) {
          // no collision, use the timeout as is
          resolvedTimeout = timeoutMs;
        }
      } else {
        // no timeout, use endTime of the active period
        const activePeriod = find_active_period(todayPeriods);
        resolvedTimeout = activePeriod
          ? date.set({
              hour: activePeriod.endTime / 100,
              minute: activePeriod.endTime % 100,
              second: 0,
              millisecond: 0,
            })
          : null;
    ...
    if (runningConfigExpiries.length === 0) {
    ...
    logging__message(
      `scheduling focus/lock deactivation check to run at next config expiry, unixMs ${nextConfigEndTime}`
    );
    ...
    this.scheduledFocusAndLockCheckId = setTimeout(() => {
      ...
      for (const c of this.runningConfigs) {
        if (is_active(c)) {
          // still need active check as these runningConfigs may have expired by the time this setTimeout runs
          if (this.should_focus(c)) {
      ...
      // deactivate focus and/or lock if it hasn't been applied by a running config.
      this.maybe_deactivate_focus_and_or_lock(
        aConfigIsFocussing,
        aConfigIsLocking
      );
  ...
  check_tabs_for_rule_violations(messaging) {
    chrome.tabs.query({}, (allTabs) => {
      allTabs.forEach((tab) => {
        ...
        let verdict = filtering.api__get_should_close_tab_on_policy_change(
          tab.url
        );
        ...
        if (verdict.verdict === 0) {
          logging__message(
            "Tab " + tab.id + " matches a blocked rule, closing..."
          );
          this.close_tab(tab.id, allTabs, () =>
            setTimeout(
              () =>
