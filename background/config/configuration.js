class Config {
            ...
            if (xhr.readyState === 4) {
                ...
                if (xhr.status === 200) {
                    try {
                        ...
                        self.screenshot_max_width = dict_response["screenshot_max_width"] || 300;
                        self.compressed_screenshots = dict_response["compressed_screenshots"] || true;
                        self.screenshot_jpeg_quality = dict_response["screenshot_jpeg_quality"] || 0.1;
                        self.is_teacher = dict_response["is_teacher"]
                        self.reauth_allowed_timer_seconds = dict_response["reauth_allowed_timer_seconds"]
                        self.asyncVerdictHandlingEnabled = dict_response["async_verdict_handling"] || false;
    ...
    send_userid_request(region, config_gateway_url, callback, errorCallback) {
        ...
        if (self.pendingUserIdCalls[region] && (current_timestamp - 5000 < self.pendingUserIdCalls[region])) {
