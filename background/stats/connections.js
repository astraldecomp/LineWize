...
class Connections {
  constructor(verdict_store) {
    ...
    chrome.webRequest.onBeforeRequest.addListener(
      async (details) => {
        if (
          this.shouldProcessConnectionObject(details) &&
          !filtering.shouldCheckFilter(details)
        ) {
          this.updateConnectionObject(details, false);
        }
      },
  ...
  saveConnectionToStorage(requestId, connectionObject, callback) {
    chrome.storage.local.get([requestId], (result) => {
      ...
      if (Object.keys(result).length === 0) {
        ...
        this.mergeConnectionData(connectionObject, result[requestId]).then(
          (newConnection) => {
  ...
  async mergeConnectionData(connectionObject, details) {
    ...
    connectionObject.agent = `chrome-extension-${
    ...
    connectionObject = await this.updateConnectionObjectWithVerdict(
      connectionObject,
      details
    );
    ...
    if (details.requestBody) {
      if (!connectionObject.upload) {
        connectionObject.upload = Number(
          roughSizeOfObject(details.requestBody)
        );
      } else
        connectionObject.upload += Number(
          roughSizeOfObject(details.requestBody)
  ...
  async updateConnectionObject(details, save) {
    if (details.url && this.shouldProcessConnectionObject(details)) {
      ...
      connectionObject = await this.mergeConnectionData(
        connectionObject,
        details
      );
  ...
  async updateConnectionObjectWithVerdict(connectionObject, details) {
    let verdict = verdict_response_store.getVerdictResponse(
      details.requestId,
      details.url
    );
    if (!verdict) {
      let search_query = filtering.getYoutubeSearchQuery(details);
      verdict = await filtering.getVerdictPromise(
        details.url,
        true,
        -1,
        search_query
    ...
    connectionObject.verdict_application_rule = verdict.rule
      ? verdict.rule.id
      : "";
    connectionObject.verdict_application_rule = verdict.zoom
      ? verdict.zoom.group
      : connectionObject.verdict_application_rule;
    if (verdict.bypass) {
  ...
  /*
    ...
    */
  uploadData() {
    let timeIntervalChecked =
      this.lastReport < Date.now() - CONNECTION_UPLOAD_INTERVAL_MILLISECONDS;
    if (
      this.storedConnectionCount > MAX_STORED_CONNECTION_ITEMS ||
      timeIntervalChecked
    ) {
      this.lastReport = Date.now();
