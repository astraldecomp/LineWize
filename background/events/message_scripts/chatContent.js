...
(function () {
  ...
  let resizeHandler = function () {
    ...
    if (!isThreadedMode) {
      let disabledNotification = document.getElementById(
...
function removeDisplayName(parent) {
  if (parent) {
    let displayName = parent.getElementsByClassName(
      "ClassroomChat_displayName"
    );
...
function createChatDisableNotification(message) {
  ...
  a.append(link);
  a.href =
    "https://dyzz9obi78pm5.cloudfront.net/app/image/id/60ffc732498ccce8297b23c8/n/cw-chat-students-using-classwize-chat.pdf";
  a.target = "_blank";
  learnMore.append(a);
...
function insertDate(parentContainer, date, insertPoint) {
  if (parentContainer) {
    ...
    let textNode = document.createTextNode(
      "For your protection, this chat service is logged and monitored by your school district."
    );
...
function refreshChatUi(classRooms) {
  if (isThreadedMode) {
    ...
    // We are in chat mode.
    const chatAllowed =
      !classRooms[currentClassroomId].chatBlocked &&
      classRooms[currentClassroomId].isActive;
    ...
    if (!chatAllowed) {
      if (classRooms[currentClassroomId].chatBlocked) {
        ...
        createChatDisableNotification(
          "Chat is disabled until your class is online again."
        );
      }
    } else {
...
function updateTotalUnreadCount() {
  ...
  chrome.runtime.sendMessage({
    type: "UPDATE_TOTAL_UNREAD_COUNT",
...
function showLoginScreen() {
  chrome.runtime.sendMessage({ message: "last_chat_message" }, (data) => {
    ...
    let loginMsgBody =
      "Sign in with your school-associated Google account to send a message";
    ...
    if (data) {
        ...
        for (classroomKey in classRooms) {
          ...
          if (currentClass.teacherInformation) {
            ...
            if (id === data.sender.id && first_name && last_name) {
              teacherName = `${capitalizeString(first_name)} ${capitalizeString(
                last_name
              )}`;
              break;
            }
      ...
      loginMsgBody =
        "Sign in with your school-associated Google account to message them back!";
...
function buildChatUiHeading() {
  ...
  classEllipse.style.backgroundColor =
    bgColors[findColorIndex() % bgColors.length];
...
function buildChatUi(newMessage) {
  ...
  if (!chatUiSkeletonRendered) {
    ...
    let updateSendIcon = function () {
      sendIcon.disabled =
        ...
        inputText.value.length >= maxMessageLength;
  ...
  let orderedData = newMessage
    ? threadChatMessages.reverse()
    : threadChatMessages.slice(loadedMessageCount);
  let conversation = document.getElementById("chatMessageConversation");
  ...
  for (let msg of orderedData) {
    ...
    const teacherIndex =
      sender === null
        ? -1
        : classRooms[currentClassroomId].teachers.indexOf(msg.sender.username);
    const date = new Date(msg.timestamp);
    let [message] = chatMessageElement(
      msg.message,
      date,
      msg.id,
      sender,
    ...
    if (!lastRenderedLoadedNode || newMessage) {
      ...
      if (!newMessage) {
        ...
        if (
          newestMessage &&
          newestMessage.sender.username === msg.sender.username
        ) {
      ...
      if (
        oldestMessage &&
        oldestMessage.sender.username === msg.sender.username
      ) {
        // Compare time stamp
    ...
    xhr.onreadystatechange = () => {
      if (xhr.readyState === 4) {
        if (xhr.status === 200) {
          ...
          let sendIndicator = document.getElementById(
            "sendIndicator" + body.id
          );
          if (sendIndicator) {
...
function sendMessage() {
  ...
  let chatMessageConversation = document.getElementById(
  ...
  if (
    ...
    date.getDay() !== newestMessageTimestamp.getDay()
  ) {
    if (!isDailyReminderDisplayed()) {
  ...
  if (newestMessageTimestamp) {
    if (
      date.getTime() - newestMessageTimestamp.getTime() <= groupingTimeLimit &&
      newestMessage.sender.username === userName
    ) {
...
function fetchChats(newMessage) {
  ...
  const teacherString =
    teachers && teachers.length ? teachers.sort().join(",") : "";
  ...
  let url =
    baseUrl +
    `/message?classroomid=${base64url(
      currentClassroomId
    )}&threadKey=${threadKey}&username=${userName}&applianceId=${applianceId}` +
    maxResults +
    sortDirection +
    startAfterQuery;
    ...
    if (xhr.readyState === 4) {
      ...
      if (xhr.status === 200) {
        ...
        if (data && data.length) {
          if (threadChatMessages.length < chatLoadMsgCount) {
            ...
            data.forEach((message) => {
              if (
                !threadChatMessages.find(
                  (threadChatMessage) => threadChatMessage.id === message.id
                )
              ) {
        ...
        if (unreadMessages.length) {
          const firstUnreadMessage = unreadMessages.sort(
            (a, b) =>
...
function updateUnreadCount(classroomId) {
  ...
  if (notificationValue) {
    ...
    const unreadDisplayValue = !unreadMessageCount
      ? ""
...
function sortChatThreadView() {
  ...
  while (switching) {
    ...
    for (i = 0; i < rows.length - 1; i++) {
      ...
      const x = rows[i].getElementsByClassName(
        "ClassroomChatroom_classStatus"
      )[0];
      const y = rows[i + 1].getElementsByClassName(
        "ClassroomChatroom_classStatus"
      )[0];
      ...
      if (!xonline && yonline) {
        ...
        const classX = rows[i].getElementsByClassName(
          "ClassroomChatroom_name"
        )[0];
        const classY = rows[i + 1].getElementsByClassName(
          "ClassroomChatroom_name"
        )[0];
...
function base64url(input) {
  return btoa(input).replace(/\-/g, "+").replace(/_/g, "/").replace(/=/g, "");
}
