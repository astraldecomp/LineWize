class Tabs {
    ...
    capture_active_tab(windowId, callback) {
        try {
            ...
            chrome.tabs.captureVisibleTab(windowId, {format: 'png'}, function (img) {
                
                if (chrome.runtime.lastError) {
                    if (chrome.runtime.lastError.message.includes("MAX_CAPTURE_VISIBLE_TAB_CALLS_PER_SECOND")) {
                        logging__error('captureVisibleTab failed, exceeded MAX_CAPTURE_VISIBLE_TAB_CALLS_PER_SECOND');
                    } else {
                ...
                sourceImage.onload = function () {
                    ...
                    const finalWidth = sourceImage.width > width ? width : sourceImage.width;
                    const finalHeight = finalWidth * (sourceImage.height / sourceImage.width);
                    ...
                    if (config.compressed_screenshots) {
                        callback(oc.toDataURL('image/jpeg', config.screenshot_jpeg_quality));
                    } else {
