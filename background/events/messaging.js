...
class Messaging {
  ...
  print_message(message, timestamp) {
    chrome.windows.getCurrent((current) => {
      ...
      if (current.type.startsWith("normal")) {
        status = true;
      }
      ...
      chrome.windows.getAll((windows) => {
        ...
        if (!status) {
          for (let window of windows) {
            if (!window.type.startsWith("normal")) {
  ...
  _print_message(message, timestamp) {
    ...
    const callback = () =>
      chrome.tabs.executeScript(
        { code: 'message="' + message + '";timestamp=' + timestamp + ";" },
        chrome.tabs.executeScript(null, {
  ...
  _print_close_tab_message(tabFavicon, timestamp) {
    ...
    const callback = () =>
      chrome.tabs.executeScript(
        {
          code: 'tabFavicon="' + tabFavicon + '";timestamp=' + timestamp + ";",
        },
        chrome.tabs.executeScript(null, {
  ...
  _print_blocked_tab_message(tabFavicon, tabId) {
    ...
    const callback = () =>
      chrome.tabs.executeScript(
        { code: 'tabFavicon="' + tabFavicon + '";blockedTabId=' + tabId + ";" },
        chrome.tabs.executeScript(null, {
