class Login {
  ...
  login() {
    ...
    if (
      !config.inside_device_network ||
      !config.google_classroom_extension_login
    ) {
      return;
    }
    ...
    if (config.google_allow_insecure_chrome) {
      chrome.identity.getProfileUserInfo(function (obejct) {
        logging__debug(
          "Chrome provided a email identity via ::getProfileUserInfo"
        );
      ...
      chrome.identity.getAuthToken({ interactive: true }, function (token) {
        if (chrome.runtime.lastError) {
          ...
          if (token !== undefined) {
            ...
            logging__message(
              "Sending request with token to chromelogin.linewize"
            );
