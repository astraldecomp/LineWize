class Updater {
  ...
  getReauthAllowedTimer() {
    return (
      config.reauth_allowed_timer_seconds || this.REAUTH_ALLOWED_TIMER_SECONDS
    );
  }
  ...
  handle_event_open_tab(event) {
    ...
    if (
      messageContent.toLowerCase().startsWith("http://") ||
      messageContent.toLowerCase().startsWith("https://")
    ) {
  ...
  post_close_tab_callback(event, tab) {
    ...
    this.messaging.print_close_tab_message(tab["favIconUrl"], ts);
  }
      ...
      chrome.tabs.query({}, (allTabs) => {
        chrome.tabs.get(parsedtabId, (tab) => {
          if (chrome.runtime.lastError) {
            ...
            tabs.close_tab(parsedtabId, allTabs, () =>
              this.post_close_tab_callback(event, tab)
            );
          }
        });
      });
  ...
  handle_event_p2p_init(event) {
    ...
    const {
      ...
      signalHost,
      signalToken,
      signalExpiryEpochSeconds,
      remotePeerId,
      ice,
    } = data;
    p2pManager.initSignaler(signalToken, channel, remotePeerId);
  ...
  handle_event_new_chat_arrive(event) {
    ...
    if (
      event.sender.id !== config.getCurrentUserInfo().user &&
      config.getClassroomChatStatus()
    ) {
  ...
  connectToEventService = () => {
    ...
    if (config.is_teacher) {
      ...
      event_types =
        "CONFIG_UPDATE,OPEN_TAB,CLOSE_TAB,MESSAGE,CLASS_STARTED,POLICY_UPDATE,INIT_P2P";
    }
    ...
    if (self.eventService === undefined && !self.pendingEventSource) {
      ...
      self.eventService = new EventSource(eventServiceUrlv2, {
      ...
      self.eventService.onerror = async function (error) {
        ...
        chrome.cookies.get(
          { name: "authorization", url: "https://linewize.net" },
          (cookie) => {
            ...
            if (!self.eventServiceRetriedWithExistingAuth && cookie) {
              ...
              console.log(
                `Retrying with existing auth in ${jitterSeconds} seconds`
              );
              window.setTimeout(
                self._retryEventServiceWithExistingAuth.bind(self),
                jitterSeconds * 1000
              );
              ...
              window.setTimeout(
                self._retryEventServiceWithNewAuth.bind(self),
                jitterSeconds * 1000
              );
            } else {
      ...
      self.eventService.addEventListener(
        "CONFIG_UPDATE",
        function (event) {
      ...
      self.eventService.addEventListener(
        ...
        function (event) {
          ...
          if (
            !eventData ||
            (eventData.threadKey && config.isClassroomEnabled())
          ) {
      ...
      self.eventService.addEventListener(
        ...
        function (event) {
          ...
          if (
            data.peerAgentId === config.chromeId &&
            config.isClassroomEnabled()
          ) {
  ...
  _retryEventServiceWithNewAuth = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to authenticate before retrying to connect to event-service, triggering events retry again after timeout"
      );
      self._retryEventServiceAfterTimeout();
    }
  };
  ...
  _retryEventServiceAfterTimeout = () => {
    const self = this;
    logging__warning(
      "v2 Event service connect error; sleeping for " +
        self.eventServiceRetryTimeoutSeconds +
        "seconds"
    );
    ...
    window.setTimeout(
      self.connectToEventService.bind(self),
      self.eventServiceRetryTimeoutSeconds * 1000
    );
  };
