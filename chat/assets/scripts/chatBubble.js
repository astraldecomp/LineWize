/* global showChatPendo:writeable msgCount:readonly imageTop:readonly imageLeft:readonly */

/**
 * This function is injected into each active tab by the Linewize
 ...
 *
 * @typedef {boolean} showChatPendo Tracks when the Classwize Chat Window is active
 * @typedef {number} msgCount Defines the number of unread messages available
 * @typedef {number} imageTop Defines the absolute pixel position of the chat bubble from the top of the page
 * @typedef {number} imageLeft Defines the absolute pixel position of the chat bubble from the left of the page
 */
(function () {
    ...
    if (!chatIcon) {
      ...
      function dragElement(element) {
        ...
        function elementDrag(e) {
          ...
          if (
            newTop < 0 ||
            newTop + 60 > window.innerHeight ||
            newLeft < 0 ||
            newLeft + 60 > window.innerWidth
          ) {
            return;
          }
      ...
      if (msgCount) {
        notificationBubble.innerText =
          msgCount > 9 ? "9+" : msgCount.toString();
  ...
  function createChatPendo() {
    ...
    if (!chatPendo) {
      ...
      pendoChatClose.src = chrome.runtime.getURL(
      ...
      pendoChatDescription.innerText =
      ...
      parentsLearnMore.href =
        "https://dyzz9obi78pm5.cloudfront.net/app/image/id/60ffc735a3abd3de297b23c6/n/cw-chat-parents-faq.pdf";
