class Filtering {
  constructor() {
    ...
    chrome.extension.onMessage.addListener(function (
      ...
      let ruleInfo =
        "?user=" +
        config.getCurrentUserInfo()["user"] +
        "&url=" +
        extractHostname(url) +
        "&deviceid=" +
        config.device_id +
      ...
      // Fallback mode - give the all clear to block if needed
        ...
        logging__message(
          "Fallback system blocking and redirecting to blocked page"
        );
        response = "redirect";
    ...
    chrome.webRequest.onBeforeRequest.addListener(
      (details) => {
        ...
        if (self.shouldCheckFilter(details)) {
          ...
          let verdictResponse = verdict_response_store.getVerdictResponse(
            details.requestId,
            details.url
          );
          ...
          if (!verdictResponse) {
            ...
            const earlyVerdict = this.getEarlyVerdict(
              domain,
              details.url,
              excludeCache,
              false
            ...
            if (!earlyVerdict) {
              ...
              if (
                ...
                const safeSearchRedirect =
                  self.enforceGoogleSafeSearchIfRequired(details.url) ||
                  self.enforceBingSafeSearchIfRequired(details.url);
              ...
              verdictResponse = self.get__verdict(
                details.url,
                false,
                details.tabId,
                search_query,
              ...
              verdict_response_store.setVerdictResponse(
                details.requestId,
                details.url,
                verdictResponse
              );
          ...
          if (verdictResponse.verdict === 0) {
            ...
            if (
              !(
                redirectUriLower.startsWith("http://") ||
                redirectUriLower.startsWith("https://")
              )
            ) {
    ...
    chrome.webRequest.onBeforeSendHeaders.addListener(
      function (details) {
        ...
        if (youtubeSafeSearchMode) {
          const safeSearcHeader = {
            name: "YouTube-Restrict",
            value: youtubeSafeSearchMode,
          };
    ...
    chrome.webRequest.onBeforeRequest.addListener(
      (details) => {
        if (!config.inside_device_network) {
          if (
            !config.getAllowInsecureChrome() &&
            !config.getCurrentUserInfo()["token"]
          ) {
            let tabId = undefined;
  ...
  handleVerdictQueue = () => {
    for (const [key, value] of Object.entries(this.verdictQueue)) {
      ...
      for (let i = 0; i < Math.min(queue.getLength(), 2); i++) {
        ...
        this.getVerdictPromise(
          details.url,
          false,
          details.tabId,
          search_query
        ).then((verdictResponse) => {
          verdict_response_store.setVerdictResponse(
            details.requestId,
            details.url,
            verdictResponse
          ...
          if (verdictResponse.verdict === 0) {
            ...
            if (
              !verdictResponse.signatures?.noise &&
              verdictResponse.signatures?.signature ===
                "sphirewall.application.porn"
            ) {
              chrome.tabs.get(details.tabId, (tab) => {
                // if we have since moved away from the page that initiated the request, ignore the verdict
                if (
                  extractHostname(details.initiator) ===
  ...
  enforceBingSafeSearchIfRequired = (url) => {
    if (url && config.isBingSafeSearchEnabled()) {
      ...
      const check =
        /\.bing\.com$/.test(parsedUrl.hostname) &&
        /^(\/search|\/videos|\/images|\/news)/.test(parsedUrl.pathname);
      if (check) {
  ...
  enforceGoogleSafeSearchIfRequired = (url) => {
    if (url && config.isGoogleSearchEnabled()) {
      const parsedUrl = new URL(url);
      const check =
        /\.google.*$/.test(parsedUrl.hostname) &&
        /^\/search/.test(parsedUrl.pathname);
  ...
  redirect_mylinewize() {
    let redirect_url =
      "https://mylinewize." + config.active_region + ".linewize.net/login";
    let token = config.getCurrentUserInfo()["token"]
      ? config.getCurrentUserInfo()["token"]
      : "";
  ...
  shouldCheckFilter(details) {
    ...
    return (
      ...
      !(
        details.initiator &&
        details.initiator.toLowerCase().startsWith("chrome")
      ) &&
      !this.isFallback()
    );
  }
  ...
  trigger_fallback() {
    ...
    logging__warning(
      "Entering Fallback Mode - trying verdict gw again at " +
        new Date(this.fallback_until)
    );
  }
  ...
  is_bypass(website) {
    ...
    if (
      hostname.includes("linewize.net") &&
      website.includes("bypass_active")
    ) {
      return true;
    }
    return false;
  ...
  getEarlyVerdict = (domain, website, excludeCache, ignoreTtl) => {
    ...
    if (this.isSearchAutoComplete(website)) {
      logging__debug(
        `Request is a search auto complete, skipping verdict`,
        website
      );
      return {
        ttl: 60,
    ...
    if (
      (website.startsWith("chrome:") ||
        website.startsWith("chrome-extension:") ||
  ...
  getVerdictCacheHit = (domain, ignoreTtl) => {
    ...
    if (
      cacheHit &&
      (ignoreTtl ||
        this.cache[domain].time_retrieved + this.cache[domain].ttl >
          nowInSeconds())
    ) {
      ...
      this.apiGetVerdictPromise(domain, path, tabId, search_query)
        .then((new_verdict) => {
          if (!new_verdict) {
            ...
            logging__warning(
              "Verdict for website was undefined",
              website,
              new_verdict
            );
          }
  ...
  isGoogleMapsVerdictBypass = (webRequestDetails) => {
    ...
    try {
      ...
      console.error(
        "caught error, skipping google maps verdict bypass check.",
        ex
      );
    }
    ...
    try {
      if (this.isYoutubePostSearch(details.url, details.method)) {
        ...
        if (details.requestBody) {
          const requestBodyString = decodeURIComponent(
      ...
      logging__warning(
        "Couldn't retrieve search query from requestBodyString. YouTube Search RequestBody changed?",
        e
      );
    }
    return search_query;
  ...
  apiGetVerdictPromise(website, path, tabId, search_query = null) {
    return new Promise((resolve, reject) => {
      ...
      if (config.getVerdictServerUrl()) {
        ...
        let chrome_version_param =
          "&cev=" + chrome.runtime.getManifest().version;
        let query_params =
          "?requested_website=" +
          website +
          "&identity=" +
        ...
        xhr.open(
          "GET",
          config.getVerdictServerUrl() + "/get/verdict" + query_params,
          true
        );
        ...
        xhr.onload = () => {
          if (xhr.status !== 200) {
            logging__warning(
              "Failed, status was " + xhr.status.toString(),
              xhr.responseText
            );
        ...
        xhr.onerror = () => {
          ...
          completed_time = start_time + 10000; // 10 seconds
          logging__message(
            "Delegating filtering check to safeguard since verdict request failed"
          );
  ...
  api__get_should_close_tab_on_policy_change(website) {
    if (config.getVerdictServerUrl()) {
      ...
      xhr.open(
        "GET",
        config.getVerdictServerUrl() +
          "/check/classroom/verdict?" +
          "identity=" +
          config.getCurrentUserInfo().user +
          "&chrome_id=" +
          config.chromeId +
